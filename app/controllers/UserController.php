...
class UserController extends BaseController
{
	...
	{
		if (!empty($id)) {
			$sp = $this->db->prepare('CALL sp_user_select(?)');
			$sp->execute(array($id));
			$op = $sp->fetch();
		} else {
			$sp = $this->db->query('CALL sp_user_select(NULL)');
			$op = $sp->fetchAll();
		}
	...
	{
		$sp = $this->db->prepare('CALL sp_user_select_by_email(?)');
	...
	{
		$sp = $this->db->prepare('CALL sp_user_hash_select(?)');
		$sp->execute(array($id));
		$op = $sp->fetchColumn();
	...
	public function postCreateUser($id, $name, $email, $password, $roleId, $departmentId)
	{
		if (!empty($id) && !empty($name) && !empty($email) && !empty($password) && !empty($roleId) && !empty($departmentId)) {
			...
			if ($op) {
				...
				$toName = $name;
				$toEmail = $email;
				$subject = 'Tracker - Welcome to ' . $this->settings['company_name'];
